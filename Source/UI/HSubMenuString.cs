...
namespace Celeste.Mod.Hyperline.UI
{
    ...
    public class HSubMenuString : HMenuScreen
    {
        ...
        public HSubMenuString() : base("string", "string")
        {
            wiggler = Wiggler.Create(0.25f, 4f);
            Position = new(0f, 1080f);
            Visible = false;
        }


        public HSubMenuString(string title, string buttonName, string startingValue, int maxValueLength, int minValueLength)
            : base(title, buttonName)
        {
            wiggler = Wiggler.Create(0.25f, 4f);
            Position = new(0f, 1080f);
            Visible = false;
        ...
        public HSubMenuString Init<T>(string value, Action<string> onValueChange) where T : Oui => Init<T>(value, onValueChange, 12, 1);

        public HSubMenuString Init<T>(string value, Action<string> onValueChange, int maxValueLength) where T : Oui => Init<T>(value, onValueChange, maxValueLength, 1);

        public HSubMenuString Init<T>(string value, Action<string> onValueChange, int maxValueLength,
            int minValueLength) where T : Oui =>
            Init(value, onValueChange, (confirm) => Overworld.Goto<T>(), maxValueLength, minValueLength);

        public HSubMenuString Init<T>(string value, Action<string> onValueChange, Action<bool> onExit,
            int maxValueLength, int minValueLength) where T : Oui =>
            Init(value, onValueChange, (confirm) =>
            {
                Overworld.Goto<T>();
                onExit?.Invoke(confirm);
            }, maxValueLength, minValueLength);

        public HSubMenuString Init(string value, Action<string> onValueChange, Action<bool> exit,
            int maxValueLength, int minValueLength)
        {
            _Value = value ?? "";
            StartingValue = _Value;
            // log
            Logger.Log(LogLevel.Error, "Hyperline", $"HSubMenuString.Init: value={value}, onValueChange={onValueChange}, exit={exit}, maxValueLength={maxValueLength}, minValueLength={minValueLength}");
        ...
        {
            // get the title and button name
            string title = (string) parameters[0];
            string buttonName = (string) parameters[1];

            // set them
            MenuName = title;
            ButtonName = buttonName;
            // convert to call init
            string value = (string) parameters[2];
            Action<string> onValueChange = (Action<string>) parameters[3];
            int maxValueLength = (int) parameters[4];
            int minValueLength = (int) parameters[5];
            // debug output all of this
            Logger.Log(LogLevel.Error, "Hyperline", $"HSubMenuString.AddOptionsToMenu: title={title}, buttonName={buttonName}, value={value}, onValueChange={onValueChange}, maxValueLength={maxValueLength}, minValueLength={minValueLength}");
            // call init
            Init(value, onValueChange, null, minValueLength, maxValueLength);

            // final param list is
            // title, buttonName, value, onValueChange, maxValueLength, minValueLength
        }

        public override string GetButtonName(object[] param)
        {
        ...
        {
            Logger.Log(LogLevel.Error, "Hyperline", $"HSubMenuString.Enter: from={from}");
            IEnumerator enterEnum = base.Enter(from);

            Logger.Log(LogLevel.Error, "Hyperline", $"Completed base.Enter(from)");
            if (UseKeyboardInput)
            {
            ...
            Logger.Log(LogLevel.Error, "Hyperline", $"HSubMenuString.Enter: letterChars={letterChars}");
            ...
            // start enterenum to initialize the menu and begin the fade in
        ...
        public override IEnumerator Leave(Oui next)
        {
            ...
            IEnumerator enumerator = base.Leave(next);
        ...
        public override void Update()
        {
            ...
            if (Input.MenuRight.Pressed && (optionsIndex < 3 || !selectingOptions) &&
                (Value.Length > 0 || !selectingOptions))
            {
                if (selectingOptions)
                {
            ...
            else if (Input.MenuDown.Pressed && !selectingOptions)
            {
                ...
                if (selectingOptions)
                {
                    ...
                    if (Value.Length == 0 || pos < cancelWidth +
                        ((offs - cancelWidth - beginWidth - backspaceWidth - spaceWidth - (widestLetter * 3f)) / 2f))
                    {
                        optionsIndex = 0;
                    }
            ...
            else if ((Input.MenuUp.Pressed || (selectingOptions && Value.Length <= 0 && optionsIndex > 0)) &&
                     (line > 0 || selectingOptions))
            {
                if (selectingOptions)
                {
                    line = letters.Length;
                    ...
                    {
                        index = (int)((offs - beginWidth - backspaceWidth - (spaceWidth / 2f) - (widestLetter * 2f)) /
                                      widestLetter);
                    }
        ...
        public override void Render()
        {
            ...
            foreach (string letter in letters)
            {
                for (int i = 0; i < letter.Length; i++)
                {
                    ...
                    DrawOptionText(letter[i].ToString(), posLetter, new(0.5f, 0.5f), scale, selected);
                    pos.X += widestLetter;
                }
            ...
            DrawOptionText(cancel,
                pos + new Vector2(0f, lineHeight + (selectingOptions && optionsIndex == 0 ? wiggle : 0f)),
                new(0f, 1f), Vector2.One * optionsScale, selectingOptions && optionsIndex == 0);
            pos.X = Boxtopleft.X + boxWidth - backspaceWidth - widestLetter - spaceWidth - widestLetter - beginWidth -
                    boxPadding;

            DrawOptionText(space,
                pos + new Vector2(0f, lineHeight + (selectingOptions && optionsIndex == 1 ? wiggle : 0f)),
                new(0f, 1f), Vector2.One * optionsScale, selectingOptions && optionsIndex == 1,
                Value.Length == 0 || !Focused);
            pos.X += spaceWidth + widestLetter;

            DrawOptionText(backspace,
                pos + new Vector2(0f, lineHeight + (selectingOptions && optionsIndex == 2 ? wiggle : 0f)),
                new(0f, 1f), Vector2.One * optionsScale, selectingOptions && optionsIndex == 2,
                Value.Length <= 0 || !Focused);
            pos.X += backspaceWidth + widestLetter;

            DrawOptionText(accept,
                pos + new Vector2(0f, lineHeight + (selectingOptions && optionsIndex == 3 ? wiggle : 0f)),
                new(0f, 1f), Vector2.One * optionsScale, selectingOptions && optionsIndex == 3,
                Value.Length < 1 || !Focused);

            ActiveFont.DrawEdgeOutline(Value, Position + new Vector2(960f, 256f), new(0.5f, 0.5f),
                Vector2.One * 2f, Color.Gray, 4f, Color.DarkSlateBlue, 2f, Color.Black);

            index = prevIndex;
        }

        private void DrawOptionText(string text, Vector2 at, Vector2 justify, Vector2 scale, bool selected,
            bool disabled = false)
        {
        ...
        protected override void GotoMenu(Overworld overworld) => overworld.Goto<HSubMenuString>();
    }
}
